...
def answer(minions):
  """
  Return an optimal ordering for minion interrogation based on
  expected interrogation time and probability of extracting an answer
  """
  # Push the minion's index
  for i, minion in enumerate(minions):
    minion.append(i)

  # Minimize time taken / probability of answer
  minions.sort(key=lambda minion: minion[0] / (minion[1] / float(minion[2])))

  # Return sorted indexes
  return [minion[3] for minion in minions]


def test_case_1():
  assert answer([[5, 1, 5], [10, 1, 2]]) == [1, 0]


def test_case_2():
  test_list = [[390, 185, 624], [686, 351, 947], [276, 1023, 1024], [199, 148, 250]]
  assert answer(test_list) == [2, 3, 0, 1]
