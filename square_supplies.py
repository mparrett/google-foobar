...
def smallest_contained_squares(n):
  """
  Return the smallest number of squares that can be added to make n
  """
  ...
  while remain > 0:
    buy = int(math.sqrt(remain))
    remain = remain - buy * buy
    if remain >= 0:
      squares = squares + 1
...
def answer(n):
  """
  Return the smallest number of gauze pads that can be bought with n coins
  by calculating the least number of squares that comprise n
  """
  return smallest_contained_squares(n)
